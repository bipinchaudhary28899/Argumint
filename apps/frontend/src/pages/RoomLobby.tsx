...
export function RoomLobby() {
  ...
  useEffect(() => {
    ...
    // NOTE: `room` intentionally excluded from deps — re-emitting room:join on
    // every state update would race the server's deduplication check.
  ...
  // ── Real-time socket events — all update context directly ─────────────────
  useEffect(() => {
    if (!socket) return;
  ...
  // Deduplicate by userId — safety net against stale DB duplicates.
                    ...
                    <div style={{ marginBottom: "1rem" }}>
                      ...
                      <div style={{ padding: "0.75rem 1rem", background: "rgba(224,242,254,0.55)", border: "1px solid rgba(14,165,233,0.2)", borderRadius: "0.625rem" }}>
                        <p style={{ color: "var(--muted)", fontSize: "0.8rem", margin: 0, textAlign: "center" }}>
                          {allReady && participants.length >= 2 ? "All ready — waiting for host…" : "Ready up to start the debate"}
                        </p>
                      </div>
                    </div>
                  )}
                      ...
                      {(!allReady || participants.length < 2) && (
                        <div style={{ padding: "0.75rem 1rem", background: "rgba(245,158,11,0.08)", border: "1px solid rgba(245,158,11,0.2)", borderRadius: "0.625rem" }}>
                          <p style={{ color: "var(--gold)", fontSize: "0.8rem", margin: 0, textAlign: "center" }}>
                            {participants.length < 2 ? "Need at least 2 players" : `${participants.length - readyCount} player(s) not ready`}
                          </p>
                        </div>
                      )}
                    </div>
